...
fetch_all_players <- function(save_path = "./data/raw/sleeper_players") {
  ...
  log_message(paste0("Fetching all players from endpoint: ", url))
  ...
  log_message(paste0("Saving player data to JSON file: ", file_name))
  ...
  log_message(paste0("Successfully saved JSON file of all players to: ", file_path))
}
...
load_all_sleeper_players <- function(file_path) {
  ...
  log_message(paste0("Loaded all players JSON file from: ", file_path))
...
get_sleeper_player_vars <- function() {
  ...
  log_message("Pulling list of all players variables names.")
...
extract_player_info <- function(players, db_path = NULL) {
  log_message("Starting extraction of player information.")

  # Retrieve the variable names to extract
  player_vars <- get_sleeper_player_vars()
  log_message("Retrieved player variable names: ", paste(player_vars, collapse = ", "))
  ...
  log_message(glue::glue("Extracting data for {n_players} players: "))
  ...
  log_message(success_str)

  if (is.null(db_path)) {
    log_message("No database path provided. Skipping database save.")
  } else {
    ...
    log_message(glue::glue("Saved extracted data to SQLite database: {db_path}"))
  }
  return(result)
}
...
load_or_fetch_sleeper_players <- function(save_path = "./data/raw/sleeper_players") {
  ...
  if (file.exists(file_path)) {
    log_message("Raw JSON file for all player data exists. Loading player data.")
    return(load_all_sleeper_players(file_path))
  } else {
    log_message("Raw JSON file for all player data not found! Fetching player data from Sleeper API.")
...
get_trending_players <- function(type, lookback_hours = 24, limit = 25) {
  ...
  log_message(glue::glue("Fetching trending players from endpoint: {url}"))
  ...
  log_message("Extracting content from trending players request.")
  content <- httr::content(response, "text")

  # Return the response content
  log_message("Converting trending players R objects to JSON.")
