...
get_user <- function(username = NULL, user_id = NULL) {
  ...
  log_message(glue::glue("Fetching user info from endpoint: {url}"))
  ...
  log_message("Extracting content from user request.")
  content <- httr::content(response, "text")

  # Convert and return JSON
  log_message("Converting user info response to R object.")
